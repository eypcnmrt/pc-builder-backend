...
namespace PcBuilderBackend.Application.Services
{
    public class StorageService : IStorageService
    {
        ...
        public StorageService(IUnitOfWork unitOfWork, ILogger<StorageService> logger)
        {
            _unitOfWork = unitOfWork;
            _logger = logger;
        }

        public async Task<IResult<PagedData<Storage>>> List(ODataQueryOptions<Storage> options, int page, int pageSize, CancellationToken ct = default)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                return Result<PagedData<Storage>>.Error("Bir hata oluştu.");
            }
        }

        public async Task<IResult<Storage>> Get(int id, CancellationToken ct = default)
        {
        ...
        public async Task<IResult<int>> Create(CreateStorageRequest request, CancellationToken ct = default)
        {
            try
            {
                var repo = _unitOfWork.GetRepository<Storage>();
        ...
        public async Task<IResult> Update(int id, UpdateStorageRequest request, CancellationToken ct = default)
        {
            try
            {
                var repo = _unitOfWork.GetRepository<Storage>();
