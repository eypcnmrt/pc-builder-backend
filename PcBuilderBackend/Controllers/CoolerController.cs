...
namespace PcBuilderBackend.Controllers
{
    ...
    public class CoolerController : ODataController
    {
        ...
        [HttpPost("/Cooler")]
        public async Task<IActionResult> Create([FromBody] CreateCoolerRequest request, CancellationToken cancellationToken) =>
            (await _service.Create(request, cancellationToken)).ToJsonResult();

        [HttpPut("/Cooler/{id:int}")]
        public async Task<IActionResult> Update(int id, [FromBody] UpdateCoolerRequest request, CancellationToken cancellationToken) =>
